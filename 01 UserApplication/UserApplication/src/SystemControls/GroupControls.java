...
public class GroupControls
{
    ...
    public void setGroupActivationState(int groupID) throws IOException, SQLException
    {
        ...
        while (result.next())
        {
            ...
            controls.setDeviceActivationState(tempID);
        }
    }
    ...
    public List<Group> showGroups() throws SQLException
    {
        ...
        String query = "SELECT g.ID g.groupName FROM groep g LEFT JOIN usergroups ug ON g.ID = ug.groupID WHERE userID = (?)";
    ...
    public List<Device> showDevicesInGroup(int groupID) throws SQLException
    {
        ...
        String query = "SELECT d.deviceID, d.deviceName, d.deviceType, d.devicePort, centraleID FROM devicegroup dg LEFT JOIN device d ON d.deviceID = dg.deviceID WHERE dg.groupID = (?)";
        PreparedStatement pstate = c.prepareStatement(query);
        pstate.setInt(1, groupID);
        ResultSet result = pstate.executeQuery();
